...
pub fn empty_fade_colors(bar_empty_rgb: RGB) -> [RGB; 3] {
    let scale = |factor: f64| -> RGB {
        (
            (bar_empty_rgb.0 as f64 * factor).round().clamp(0.0, 255.0) as u8,
            (bar_empty_rgb.1 as f64 * factor).round().clamp(0.0, 255.0) as u8,
            (bar_empty_rgb.2 as f64 * factor).round().clamp(0.0, 255.0) as u8,
        )
    };
